...
interface DevVelocityProps {
  data: { date: string; open: number; completed: number }[];
}
...
function DevTooltip({ data, isAutoCycling, cycleKey }: { data: TooltipData; isAutoCycling: boolean; cycleKey: number }) {
  ...
  const animationName = isAutoCycling ? 'devTooltipCycle' : 'tooltipFadeIn';
...
export default function DevVelocity({ data: devTasks }: DevVelocityProps) {

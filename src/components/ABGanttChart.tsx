...
interface TooltipData {
  ...
  progress: number;
  color: string;
  flipped?: boolean; // true = show below the bar instead of above
}

const AUTO_CYCLE_INTERVAL = 4000; // 4 seconds
const TOOLTIP_HEIGHT = 100; // Approximate tooltip height for flip detection
...
function GanttTooltip({ data, isAutoCycling, cycleKey }: { data: TooltipData; isAutoCycling: boolean; cycleKey: number }) {
  ...
  const isFlipped = data.flipped;
  const animationName = isAutoCycling
    ? (isFlipped ? 'abGanttTooltipCycleFlipped' : 'abGanttTooltipCycle')
    : (isFlipped ? 'tooltipFadeInFlipped' : 'tooltipFadeIn');
  ...
  return createPortal(
    <div
      ...
      style={{
        position: 'fixed',
        left: data.x,
        top: isFlipped ? data.y + 20 : data.y - 10,
    ...
    >
      <style>{`
        ...
        @keyframes abGanttTooltipCycleFlipped {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem', fontSize: '0.55rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <span style={{ color: '#6b7082' }}>Phase:</span>
          <span style={{ color: '#e8eaf0' }}>{data.phase}</span>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
      ...
      {/* Arrow - points up when flipped, down when normal */}
      <div style={{
        position: 'absolute',
        ...(isFlipped ? {
          bottom: '100%',
...
export default function ABGanttChart({ items }: ABGanttChartProps) {
  ...
  useEffect(() => {
    ...
    if (containerRef.current) {
      ...
      // Flip tooltip if it would exceed top of viewport
      const shouldFlip = y - TOOLTIP_HEIGHT < 20;
